...
function Table() {
 ...
 return (
  <div className="w-[490px]">
   {socketData.length > 0 && ticker ? <Test data={socketData[0]} /> : <div>Orderbook Loading...</div>}
  </div>
 )
}
...
function Test(props: TestProps) {
 ...
 return (
  <div>
   {data && (
    <div>
     <div>코인 : {data.code}</div>
     <div>총 매도 물량 : {data.total_ask_size}</div>
     <div>총 매수 물량 : {data.total_bid_size}</div>
